...
def get_red(slider_value):
    ...
    #Turn the slider value into an int value and hex value. Strip leading chars so only two remain
...
def store_color():
    """Store the current color tuple value and display mode"""
...
#Put labels, sliders and button on to the frame...Use ipadx with rgb buttons to define column width, then use sticky on others
...
for i in range(6):
    ...
    stored_colors[stored_color.get()] = [new_color_tuple.cget('text'), new_color_hex.cget('text')]
    print(stored_colors[stored_color.get()])
