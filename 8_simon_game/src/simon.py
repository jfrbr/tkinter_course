...
#Define functions
def pick_sequence():
    """Pick the next value in the sequence. Do not allow for repeated value"""
    while True:
...
def check_round():
    """Determine if the player entered the correct sequence button presses for a round"""
    ...
    #The player is incorrect so change label, update score, disable buttons, and reset for the new game
    else:
        change_label("Wrong!")
        score = 0
        disable()
        #The game is over then change the message
        game_sequence = []
...
tkinter.Radiobutton(game_frame, text="Easy", variable=difficulty, value="Easy", font=game_font2, bg=game_color, command=set_dificulty).grid(row=2, column=1)
tkinter.Radiobutton(game_frame, text="Medium", variable=difficulty, value="Medium", font=game_font2, bg=game_color, command=set_dificulty).grid(row=2, column=2)
tkinter.Radiobutton(game_frame, text="Hard", variable=difficulty, value="Hard", font=game_font2, bg=game_color, command=set_dificulty).grid(row=2, column=3)
